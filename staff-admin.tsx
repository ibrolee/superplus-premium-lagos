...
function StaffAdminPage() {
  ...
  const [generalAttendanceLoading, setGeneralAttendanceLoading] = useState(false);
  ...
  async function loadStaff() {
    ...
    setLoading(false);
    void loadGeneralAttendance(attendanceDate);
  }
  ...
  async function loadGeneralAttendance(date: string) {
    ...
    setGeneralAttendanceLoading(true);
    ...
    if (attendanceError) {
      ...
      setGeneralAttendance((data || []) as GeneralAttendanceRecord[]);
    }

    setGeneralAttendanceLoading(false);
  }
  ...
  async function deleteStaff(profile: StaffProfile) {
    ...
    setStaff((current) => current.filter((member) => member.id !== profile.id));
  ...
  return (
    <main className="min-h-screen min-w-0 overflow-x-hidden bg-background">
      ...
      <div className="mx-auto max-w-7xl min-w-0 overflow-hidden px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        {!loading && (
          <nav className="mb-6 overflow-x-auto rounded-xl border border-border bg-card p-2">
        ...
        {!loading && (
          <details id="revenue" className="mb-6 rounded-xl border border-border bg-card">
                  ...
                  <label className="w-full sm:w-auto">
                    ...
                    <input
                      ...
                      onChange={(event) => {
                        ...
                        void loadGeneralAttendance(event.target.value);
                      }}
                      className="mt-2 h-11 w-full border border-border bg-background px-3 sm:w-52"
                    />
                  </label>
                </div>
