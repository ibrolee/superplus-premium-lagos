-- STAGED ONLY. Do not apply to production until UI, Edge, manager reconciliation and scanner tests pass.
-- Registration activates provisional access immediately. Pending payments are not confirmed revenue.
CREATE TABLE public.reception_instant_registrations (
 ...
 staff_reference text, staff_note text NOT NULL,
 ...
 reconciled_by uuid REFERENCES auth.users(id), reconciled_at timestamptz,
 reconciled_reference text, reconciliation_note text,
 UNIQUE(registered_by,idempotency_key), UNIQUE(membership_id), UNIQUE(payment_id)
);
...
CREATE POLICY reception_instant_registration_read ON public.reception_instant_registrations
 ...
 WHERE s.auth_user_id=(SELECT auth.uid()) AND s.active IS TRUE AND
 (lower(s.role) IN('admin','owner','manager') OR (lower(s.role)='reception' AND registered_by=(SELECT auth.uid())))));

-- Service-role only, invoked by an Edge Function after validating the actual staff JWT.
...
DECLARE v_plan public.membership_plans%ROWTYPE; v_fee numeric; v_email text; v_phone text;
 v_member uuid; v_membership uuid; v_payment uuid; v_registration uuid;
 v_existing public.reception_instant_registrations%ROWTYPE;
BEGIN
 ...
 RAISE EXCEPTION 'Active reception account required.' USING ERRCODE='42501'; END IF;
 ...
 IF length(btrim(COALESCE(p_full_name,'')))<3 OR v_email NOT LIKE '%@%.%' OR length(v_phone) NOT BETWEEN 10 AND 15 OR length(btrim(COALESCE(p_staff_note,'')))<12 THEN RAISE EXCEPTION 'Full name, valid email/phone and staff collection note (12+ characters) required.'; END IF;
 IF p_funds_confirmed IS DISTINCT FROM TRUE THEN RAISE EXCEPTION 'Personally check actual cash, POS result or gym bank credit.'; END IF;
 IF p_method NOT IN('Cash','POS','Bank Transfer') THEN RAISE EXCEPTION 'Choose cash, POS or bank transfer.'; END IF;
 IF p_method<>'Cash' AND length(btrim(COALESCE(p_staff_reference,'')))<6 THEN RAISE EXCEPTION 'POS and bank transfers need actual transaction reference. Screenshot alone is insufficient.'; END IF;
 ...
 IF p_plan_amount IS NULL OR p_plan_amount<=0 OR p_plan_amount>100000000 OR (v_plan.name<>'Custom Plan' AND p_plan_amount IS DISTINCT FROM v_plan.price) THEN RAISE EXCEPTION 'Plan amount must match official price.'; END IF;
 ...
 RAISE EXCEPTION 'Existing pending intake; resolve it before registering.'; END IF;
 INSERT INTO public.members(full_name,email,phone,source,notes)
 VALUES(btrim(p_full_name),v_email,btrim(p_phone),'manual','Instant front-desk registration by '||p_actor_id::text||'. Payment awaiting reconciliation.') RETURNING id INTO v_member;
 ...
 -- PENDING is excluded from successful revenue; do not set revenue_excluded=true because its
 -- historic-payment protection trigger would prevent non-admin managers from reconciling.
 INSERT INTO public.payments(member_id,membership_id,amount,currency,status,payment_method,provider,metadata,source)
 VALUES(v_member,v_membership,p_plan_amount+v_fee,'NGN','pending',p_method,'manual_reception',jsonb_build_object('collection_status','awaiting_reconciliation','registered_by_auth_id',p_actor_id,'registration_amount',v_fee,'membership_amount',p_plan_amount,'staff_reference',nullif(btrim(COALESCE(p_staff_reference,'')),''),'staff_note',btrim(p_staff_note),'funds_confirmation','staff_attested_not_independently_verified'),'reception_instant_provisional') RETURNING id INTO v_payment;
...
-- Only a different active manager may reconcile the transaction AFTER immediate registration.
...
BEGIN
 ...
 IF p_action NOT IN('reconcile','dispute') OR length(btrim(COALESCE(p_review_note,'')))<12 THEN RAISE EXCEPTION 'Choose reconcile/dispute and add independent notes of 12+ characters.'; END IF;
 ...
 IF p_action='reconcile' THEN
  ...
  IF length(v_ref)<6 THEN RAISE EXCEPTION 'Provide unique verified bank/POS or cash-count/deposit reference.'; END IF;
  IF EXISTS(SELECT 1 FROM public.payments WHERE lower(btrim(COALESCE(paystack_reference,'')))=v_ref) THEN RAISE EXCEPTION 'Do not reuse Paystack reference.'; END IF;
  ...
  UPDATE public.payments SET status='success',provider='manual_verified',paid_at=clock_timestamp(),metadata=COALESCE(metadata,'{}'::jsonb)||jsonb_build_object('collection_status','reconciled','independent_verified_reference',v_ref,'reconciled_by_auth_id',(SELECT auth.uid()),'reconciliation_note',btrim(p_review_note)) WHERE id=v_entry.payment_id AND status='pending';
  IF NOT FOUND THEN RAISE EXCEPTION 'Payment changed; investigate before retrying.'; END IF;
  UPDATE public.memberships SET payment_status='paid' WHERE id=v_entry.membership_id AND status='active';
  IF NOT FOUND THEN RAISE EXCEPTION 'Membership changed; investigate before retrying.'; END IF;
