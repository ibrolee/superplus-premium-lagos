...
// STAGED SOURCE ONLY: never deploy to the live Supabase project until paired verifier is tested.
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  const key=Deno.env.get('PAYSTACK_SECRET_KEY');if(!key)return respond({error:'Payment system is not configured.'},503);
