...
type Action = { label: string; href: string; description: string; keywords: string; management?: boolean; reception?: boolean };
const actions: Action[] = [
  ...
  { label: "Reception dashboard", href: "/reception-dashboard", description: "Original reception and member tools", keywords: "member register renew manage membership", reception: true },
...
export function ActionSearch({ role }: { role: string }) {
  ...
  const matches = useMemo(() => {
    ...
    return actions.filter((action) => (!action.management || management) && (!action.reception || reception)).filter((action) => words.every((word) => `${action.label} ${action.description} ${action.keywords}`.toLowerCase().includes(word)));
  }, [query, management, reception]);
  return <div ref={root} className="relative w-full max-w-sm" role="search" aria-label="Search staff and reception actions">
    ...
    {open && <div id="staff-action-results" className="absolute left-0 right-0 top-full z-[100] mt-2 max-h-[min(65vh,420px)] overflow-y-auto rounded-2xl border border-[#d9e6d2] bg-white p-2 text-[#193d2b] shadow-2xl"><p className="px-3 py-2 text-[11px] font-bold uppercase tracking-wider text-[#718172]">{query.trim() ? `${matches.length} matching actions` : "Quick actions"}</p>{matches.map((action) => <a key={action.href} href={action.href} onClick={() => setOpen(false)} className="block rounded-xl px-3 py-2.5 hover:bg-[#edf6e9] focus:bg-[#edf6e9] focus:outline-none"><span className="block text-sm font-bold">{action.label}</span><span className="mt-0.5 block text-xs text-[#667668]">{action.description}</span></a>)}{matches.length === 0 && <p className="px-3 py-5 text-sm text-[#667668]">No matching actions. Try “member”, “salary” or “scan”.</p>}</div>}
  </div>;
}
