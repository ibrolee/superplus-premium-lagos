...
export function ActionSearch({ role }: { role: string }) {
  ...
  return (
    <div
      ...
      role="search"
      aria-label="Search available portal tools"
    >
      <div className="flex items-center gap-2 rounded-xl border border-white/25 bg-white/10 px-3 text-white focus-within:border-[#b8ee73] focus-within:ring-2 focus-within:ring-[#b8ee73]/30">
        ...
        {open && (
          <button
            ...
            className="rounded px-2 py-1 text-xs font-bold text-[#d3f9a4] hover:bg-white/10"
          >
            Close
          </button>
        )}
      ...
      {open && (
        <section
          id="staff-action-results"
          aria-label="Action search results"
          className="mt-3 max-h-[min(54dvh,460px)] overflow-y-auto overscroll-contain rounded-2xl border border-[#d9e6d2] bg-white p-2 text-[#193d2b] shadow-xl"
        >
          <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-2">
            <p
              role="status"
              className="text-[11px] font-bold uppercase tracking-wider text-[#718172]"
            >
              {query.trim() ? `${matches.length} matching actions` : "Suggested actions"}
            </p>
          ...
          {matches.map((action) => (
            <a
              ...
              href={action.href}
              className="flex items-center justify-between gap-3 rounded-xl px-3 py-3 hover:bg-[#edf6e9] focus:bg-[#edf6e9] focus:outline-2 focus:outline-[#4b854b]"
            >
              <span className="min-w-0">
                <span className="mb-1 block text-[10px] font-bold uppercase tracking-wide text-[#54815c]">
                  {action.category}
                </span>
                <span className="block text-sm font-bold">{action.label}</span>
                <span className="mt-0.5 block text-xs leading-5 text-[#667668]">
                  {action.description}
                </span>
              </span>
          ...
          {matches.length === 0 && (
            <p className="px-3 py-5 text-sm text-[#667668]">
              No matching tools found. Try “attendance”, “members”, “salary”, “blog” or “gallery”.
            </p>
          )}
        </section>
      )}
