...
export function WorkspaceNavigation() {
  ...
  if (onReception || onStaff)
    return (
      <nav
        aria-label="Staff and reception action navigation"
        className="relative z-30 border-b border-[#365139] bg-[#173326] px-4 py-3 text-white"
      >
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3">
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-3">
            <span className="text-xs font-black uppercase tracking-wider text-[#b8ee73]">
              Staff tools
            </span>
            {role && <ActionSearch role={role} />}
          </div>
          <div className="flex flex-wrap gap-2 text-xs font-bold">
            {reception && (
              <a
                href="/reception-workspace"
                aria-current={pathname === "/reception-workspace" ? "page" : undefined}
                className={`rounded-lg border px-3 py-2 ${pathname === "/reception-workspace" ? "border-[#b8ee73] bg-[#b8ee73] text-[#173326]" : "border-white/25 hover:bg-white/10"}`}
              >
                Reception 2.0
              </a>
            )}
            ...
            <a
              ...
              aria-current={pathname === "/staff-missed-scans" ? "page" : undefined}
              className={`rounded-lg border px-3 py-2 ${pathname === "/staff-missed-scans" ? "border-[#b8ee73] bg-[#b8ee73] text-[#173326]" : "border-white/25 hover:bg-white/10"}`}
            >
              Missed scan / requests
            </a>
            {admin && (
              <a
            ...
            {reception && (
              <a
                ...
                className="inline-flex items-center gap-1 rounded-lg bg-[#b8ee73] px-3 py-2 text-[#173326] hover:bg-[#c9f69c]"
              >
                <Cake size={14} /> Reminders <ArrowUpRight size={13} />
              </a>
            )}
          </div>
        </div>
  ...
  return (
    <nav
      aria-label="Super Plus management workspace"
      className="relative z-30 border-b border-[#263d31] bg-[#152820] px-3 py-3 text-white sm:px-6"
    >
      <div className="mx-auto flex max-w-[1680px] flex-wrap items-center gap-x-5 gap-y-3">
        <span className="hidden shrink-0 text-[10px] font-black uppercase tracking-[.19em] text-[#b8ee73] sm:block">
        ...
        >
          {pages.map(({ label, href, icon: Icon }) => (
            <a
              ...
              className={`inline-flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold transition-colors ${pathname === href ? "bg-[#b8ee73] text-[#183125]" : "bg-white/5 text-[#d5e3d8] hover:bg-white/15"}`}
            >
              <Icon size={15} />
              {label}
            </a>
        ...
        <div className="flex shrink-0 items-center gap-2 overflow-x-auto text-xs">
          <a
            ...
            className="inline-flex items-center gap-1.5 rounded-lg border border-white/20 px-3 py-2 font-semibold hover:bg-white/10"
          >
            <ScanLine size={15} /> Scanner <ArrowUpRight size={13} />
          </a>
          {management && (
            <a
              href="/staff-admin"
              className="inline-flex items-center gap-1.5 rounded-lg border border-white/20 px-3 py-2 font-semibold hover:bg-white/10"
            >
              <Wallet size={15} /> Admin <ArrowUpRight size={13} />
            </a>
          )}
        </div>
        <div className="w-full sm:ml-auto sm:w-80">
          <ActionSearch role={role} />
        </div>
      </div>
    </nav>
  );
