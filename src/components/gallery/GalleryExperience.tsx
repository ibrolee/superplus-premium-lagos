...
type GalleryItem = { id: string; title: string; category: string; media_type: 'image' | 'video'; storage_path: string; thumbnail_path: string | null; sort_order: number; created_at: string };
type DisplayItem = { id: string; title: string; category: string; media_type: 'image' | 'video'; url: string; thumbnailUrl: string };
...
function useGallery() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const { data, error } = await supabase.from('gallery_media').select('id,title,category,media_type,storage_path,thumbnail_path,sort_order,created_at').eq('is_published', true).order('sort_order', { ascending: true }).order('created_at', { ascending: false });
        if (!alive) return;
        if (error) console.error('Unable to load gallery media', error);
        else setItems(((data || []) as GalleryItem[]).map(item => {
          const bucket = supabase.storage.from('gallery-media');
          const url = bucket.getPublicUrl(item.storage_path).data.publicUrl;
          return { id: item.id, title: item.title, category: item.category, media_type: item.media_type, url, thumbnailUrl: item.media_type === 'image' && item.thumbnail_path ? bucket.getPublicUrl(item.thumbnail_path).data.publicUrl : url };
        }));
      } catch (error) {
...
function GalleryImage({ item, className, priority }: { item: DisplayItem; className: string; priority: boolean }) {
  const [source, setSource] = useState(item.thumbnailUrl);
  const [failed, setFailed] = useState(false);
  return failed ? <div className="flex h-full w-full items-center justify-center bg-zinc-800 px-3 text-center text-xs text-white/70" role="status">Photo temporarily unavailable. Tap to retry.</div> : <img src={source} alt="" loading={priority ? 'eager' : 'lazy'} fetchPriority={priority ? 'high' : 'auto'} decoding="async" referrerPolicy="no-referrer" className={className} onError={() => source !== item.url ? setSource(item.url) : setFailed(true)} />;
}

function MediaTile({ item, onOpen, priority }: { item: DisplayItem; onOpen: () => void; priority: boolean }) {
  return <button type="button" onClick={onOpen} aria-label={`View ${item.title}`} className="group relative block aspect-[4/5] w-full overflow-hidden rounded-[1.4rem] bg-zinc-800 text-left shadow-xl focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary sm:aspect-[5/4]">
    {item.media_type === 'video' ? <video src={`${item.url}#t=0.1`} preload="metadata" muted playsInline className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <GalleryImage key={item.thumbnailUrl} item={item} priority={priority} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />}
...
export function GalleryExperience({ compact = false }: { compact?: boolean }) {
  ...
  return <section className="overflow-hidden bg-[#191615] py-16 text-white sm:py-24" aria-label="Super Plus Fitness Gallery">
    <div className="section-shell">
      ...
      {loading ? <div className="rounded-3xl border border-white/15 bg-white/5 px-6 py-16 text-center text-sm text-white/65" role="status">Loading gallery…</div> : visible.length ? <div className={`grid gap-3 sm:gap-4 ${compact ? 'grid-cols-2 lg:grid-cols-4' : 'grid-cols-2 md:grid-cols-3'}`}>{visible.map((item,index) => <MediaTile key={item.id} item={item} priority={index<4} onOpen={() => setSelected(item)} />)}</div> : <div className="rounded-3xl border border-white/15 bg-white/5 px-6 py-16 text-center"><Images className="mx-auto mb-3 size-9 text-primary" /><p className="text-lg font-semibold">No photos or videos published yet.</p><p className="mt-2 text-sm text-white/50">Check back as our gallery grows.</p></div>}
