...
export function GalleryExperience({ compact = false }: { compact?: boolean }) {
  ...
  return <section className="overflow-hidden bg-[#191615] py-16 text-white sm:py-24" aria-label="Super Plus Fitness Gallery">
    <div className="section-shell">
      <div className="mb-9 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="mb-3 text-xs font-bold uppercase tracking-[.23em] text-primary">Inside Super Plus Fitness</p><h1 className="font-display text-5xl font-black uppercase leading-[.92] tracking-tight sm:text-7xl">See the space.<br /><span className="text-primary">Feel the energy.</span></h1><p className="mt-5 max-w-lg text-sm leading-7 text-white/65">Explore our facilities, equipment, classes and moments from the gym.</p></div>{compact && <Button asChild size="lg" variant="inverse"><Link to="/gallery">Explore full gallery <ArrowRight /></Link></Button>}</div>
      ...
      {compact && <Button asChild size="lg" variant="inverse" className="mt-8 w-full sm:hidden"><Link to="/gallery">View the gallery <ArrowRight /></Link></Button>}
