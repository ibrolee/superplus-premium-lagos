...
function EditorialArtwork({ post, featured = false }: { post: BlogPost; featured?: boolean }) {
  ...
  return (
    <div className="relative flex h-full w-full flex-col justify-between overflow-hidden bg-[#232321] p-6 text-white sm:p-8" aria-hidden="true">
      ...
      <div className="relative flex items-center justify-between text-[10px] font-extrabold uppercase tracking-[0.2em] text-white/70"><span>Super Plus / Journal</span><span>Est. Lagos</span></div>
      ...
      <p className="relative z-10 text-[10px] font-extrabold uppercase tracking-[0.22em] text-white/70">{post.category} / Field notes</p>
    </div>
  );
}
...
export default function BlogIndexV2() {
  ...
  return <div className="spf-blog-index min-h-screen bg-[#f7f5f0] text-[#262521]">
    <section className="relative isolate overflow-hidden bg-[#20211e] text-white" aria-labelledby="blog-heading">
      ...
      <div className="mx-auto grid max-w-[1440px] lg:min-h-[560px] lg:grid-cols-[1.04fr_0.96fr]">
        <div className="relative z-10 flex flex-col justify-center px-5 pb-14 pt-16 sm:px-10 sm:py-20 lg:px-16 lg:py-24 xl:px-24">
          <div className="mb-9 flex items-center gap-3 text-[10px] font-extrabold uppercase tracking-[0.24em] text-[#f16a56]"><span className="h-px w-8 bg-[#f16a56]" /> The Super Plus Journal <span className="text-white/40">/ 02</span></div>
          ...
          <p className="mt-9 max-w-[490px] text-sm leading-7 text-white/65 sm:text-base">Ideas worth moving for. Straightforward stories about training, recovery, nutrition and building a stronger everyday life.</p>
          <a href="#stories" className="mt-10 inline-flex w-fit items-center gap-3 border-b border-[#f05a46] pb-2 text-xs font-extrabold uppercase tracking-[0.15em] transition-colors hover:text-[#f05a46] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#f05a46] motion-reduce:transition-none">Explore the stories <ArrowDown className="size-4" aria-hidden="true" /></a>
        </div>
      ...
      <div className="relative z-10 border-t border-white/10 bg-[#171816]"><div className="mx-auto flex max-w-[1440px] items-center justify-between gap-6 overflow-x-auto px-5 py-4 text-[10px] font-bold uppercase tracking-[0.17em] text-white/55 sm:px-10 lg:px-16 xl:px-24"><span className="shrink-0 text-[#f05a46]">Explore the journal</span><span className="shrink-0">01 / Fitness</span><span className="shrink-0">02 / Nutrition</span><span className="shrink-0">03 / Recovery</span><span className="shrink-0">04 / Wellness</span></div></div>
    </section>

    <section id="stories" className="scroll-mt-24 border-b border-[#e5e1d9] bg-[#f7f5f0]" aria-label="Find articles">
      <div className="mx-auto max-w-[1280px] px-5 py-9 sm:px-10 sm:py-11">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div><span className="text-[10px] font-extrabold uppercase tracking-[0.21em] text-[#db4d39]">Find your focus</span><h2 className="mt-1 font-display text-4xl font-bold uppercase tracking-tight sm:text-5xl">The reading room<span className="text-[#df4c38]">.</span></h2></div>
    ...
    <main className="mx-auto max-w-[1280px] px-5 pb-20 pt-12 sm:px-10 sm:pb-28 sm:pt-16">
      ...
      : error ? <div role="alert" className="mx-auto max-w-xl border border-[#e2d8d0] bg-white px-8 py-16 text-center"><BookOpen className="mx-auto size-10 text-[#db4d39]" /><h2 className="mt-5 font-display text-3xl font-bold uppercase">Couldn't load the journal</h2><p className="mt-3 text-sm text-[#716e67]">Please check your connection and try again.</p><button type="button" onClick={() => setAttempt(n => n + 1)} className="mt-7 min-h-11 bg-[#df4c38] px-6 text-xs font-bold uppercase tracking-widest text-white">Try again</button></div>
      : !lead ? <div className="mx-auto max-w-2xl border border-[#e4e0d8] bg-white px-6 py-20 text-center"><BookOpen className="mx-auto size-10 text-[#df4c38]" /><h2 className="mt-5 font-display text-4xl font-bold uppercase">{posts.length === 0 ? "Good things are in the works." : "No matching stories."}</h2><p className="mt-3 text-sm leading-7 text-[#716e67]">{posts.length === 0 ? "We're preparing useful, practical stories for the Super Plus community." : "Try a different search term or explore all topics."}</p>{hasFilters && <button type="button" className="mt-7 min-h-11 border border-[#df4c38] px-6 text-xs font-extrabold uppercase tracking-widest text-[#df4c38]" onClick={() => {setSearch(""); setTopic("All");}}>Clear filters</button>}</div>
      : <>
        <section aria-labelledby="featured-heading" className="mb-16 sm:mb-24">
          <div className="mb-6 flex flex-wrap items-end justify-between gap-3 border-b border-[#dedbd3] pb-4"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.22em] text-[#db4d39]">Editor's selection / 01</p><h2 id="featured-heading" className="mt-1 font-display text-4xl font-bold uppercase tracking-tight sm:text-5xl">The lead story<span className="text-[#df4c38]">.</span></h2></div><span className="text-[11px] font-semibold text-[#817e77]">{filtered.length} {filtered.length === 1 ? "story" : "stories"} in this selection</span></div>
          <Link to="/blog/$slug" params={{ slug: lead.slug }} className="group grid overflow-hidden border border-[#dedbd3] bg-white shadow-[0_10px_35px_rgba(35,32,28,.04)] outline-offset-4 transition-all duration-300 hover:border-[#df4c38]/45 hover:shadow-[0_18px_50px_rgba(35,32,28,.12)] focus-visible:outline-2 focus-visible:outline-[#df4c38] motion-reduce:transition-none lg:min-h-[435px] lg:grid-cols-[1.08fr_0.92fr]">
            <div className="relative min-h-[320px] overflow-hidden sm:min-h-[400px] lg:min-h-full"><EditorialArtwork post={lead} featured /><span className="absolute bottom-4 right-4 z-10 bg-[#df4c38] px-3 py-2 text-[10px] font-extrabold uppercase tracking-[0.16em] text-white">Featured story ↗</span></div>
            <div className="flex flex-col justify-center px-6 py-9 sm:px-10 sm:py-12 lg:px-12"><span className="mb-6 flex items-center gap-2 text-[10px] font-extrabold uppercase tracking-[0.22em] text-[#db4d39]"><span className="h-px w-6 bg-[#df4c38]" />{lead.category}</span><Meta post={lead} /><h3 className="mt-5 font-display text-[clamp(2.6rem,4.2vw,4.8rem)] font-bold uppercase leading-[0.92] tracking-[-0.025em] transition-colors group-hover:text-[#cf412e] motion-reduce:transition-none">{lead.title}</h3>{lead.excerpt && <p className="mt-6 line-clamp-3 text-sm leading-7 text-[#706d66] sm:text-base">{lead.excerpt}</p>}<div className="mt-9 inline-flex items-center gap-3 text-xs font-extrabold uppercase tracking-[0.15em] text-[#c7402e]">Read the full story <span className="flex size-10 items-center justify-center rounded-full border border-[#df4c38]/35 transition-transform group-hover:translate-x-1 motion-reduce:transition-none"><ArrowRight className="size-4" /></span></div></div>
          </Link>
        </section>
        {remaining.length > 0 && <section aria-labelledby="latest-heading"><div className="mb-7 flex flex-wrap items-end justify-between gap-4 border-b border-[#dedbd3] pb-4"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.22em] text-[#db4d39]">Keep exploring / 02</p><h2 id="latest-heading" className="mt-1 font-display text-4xl font-bold uppercase sm:text-5xl">More to read<span className="text-[#df4c38]">.</span></h2></div><p className="text-xs text-[#817e77]">Fresh perspectives from Super Plus</p></div><div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{remaining.map(post => <Link key={post.id} to="/blog/$slug" params={{ slug: post.slug }} className="group flex h-full flex-col overflow-hidden border border-[#dedbd3] bg-white outline-offset-4 transition-all duration-300 hover:-translate-y-1 hover:border-[#df4c38]/40 hover:shadow-xl focus-visible:outline-2 focus-visible:outline-[#df4c38] motion-reduce:transform-none motion-reduce:transition-none"><div className="relative aspect-[16/10] overflow-hidden"><EditorialArtwork post={post} /><span className="absolute bottom-3 left-3 bg-white px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-widest text-[#cf412e]">{post.category}</span></div><div className="flex flex-1 flex-col p-6"><Meta post={post} /><h3 className="mt-4 font-display text-3xl font-bold uppercase leading-[1] tracking-tight transition-colors group-hover:text-[#cf412e]">{post.title}</h3>{post.excerpt && <p className="mt-4 line-clamp-3 text-sm leading-6 text-[#716e67]">{post.excerpt}</p>}<div className="mt-auto flex items-center justify-between border-t border-[#e9e5df] pt-5 text-[11px] font-extrabold uppercase tracking-[0.12em] text-[#c7402e]"><span>Read story</span><ArrowRight className="size-5 transition-transform group-hover:translate-x-1 motion-reduce:transition-none" /></div></div></Link>)}</div></section>}
      </>}
    </main>
    <section className="relative overflow-hidden bg-[#232321] text-white"><div className="pointer-events-none absolute -right-20 -top-24 size-80 rounded-full border-[55px] border-[#df4c38]/25" aria-hidden="true" /><div className="relative mx-auto flex max-w-[1280px] flex-col items-start justify-between gap-7 px-5 py-16 sm:px-10 sm:py-20 lg:flex-row lg:items-center"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.23em] text-[#f16a56]">From inspiration to action</p><h2 className="mt-3 font-display text-5xl font-bold uppercase leading-[0.95] sm:text-6xl">Take your next step<span className="text-[#f05a46]">.</span></h2><p className="mt-4 max-w-xl text-sm leading-7 text-white/60">Turn what you read into a routine that works for you. Explore our gym membership and training options.</p></div><Link to="/membership" className="inline-flex min-h-12 shrink-0 items-center justify-center gap-4 bg-[#e44b37] px-7 text-xs font-extrabold uppercase tracking-[0.15em] text-white transition-colors hover:bg-[#c93d2c] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white motion-reduce:transition-none">Explore membership <ArrowRight className="size-4" aria-hidden="true" /></Link></div></section>
  </div>;
}
