...
async function isAllowed(userId: string, authenticatedEmail: string | undefined, portal: Portal): Promise<boolean> {
  const { data: account, error } = await supabase.from("staff_users").select("role,active").eq("auth_user_id", userId).maybeSingle();
  if (error) throw error;
  const role = String(account?.role || "").toLowerCase();
  if (portal !== "staff") {
    return account?.active === true && role === portal && authenticatedEmail?.trim().toLowerCase() === fixedEmail[portal];
  }
  ...
  const { data: profile, error: profileError } = await supabase.from("staff_profiles").select("id,status").eq("auth_user_id", userId).maybeSingle();
...
export function PortalAuth({ portal }: { portal: Portal }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return;
        if (allowed) { window.location.replace(destination[portal]); return; }
        setOtherSession(true);
      } catch (cause) {
        if (!cancelled) setError(cause instanceof Error ? cause.message : "Unable to verify this session.");
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (fixedEmail[portal] && cleanEmail !== fixedEmail[portal]) throw new Error("This email is not assigned to this portal.");
      if (recovery) {
      ...
      if (register) {
        if (portal !== "staff") throw new Error("Reception and admin accounts cannot self-register.");
        ...
        const { data, error: signupError } = await supabase.auth.signUp({
          ...
          options: { emailRedirectTo: `${window.location.origin}/portal/staff`, data: { account_type: "staff", full_name: name.trim(), phone: phone.trim() } },
        });
        if (signupError) throw signupError;
        setPassword("");
        setNotice(data.session ? "Application submitted. Management approval is required before work access." : "Application submitted. Check your email if verification is required; management approval is also required.");
        return;
      }
      ...
      if (!(await isAllowed(data.user.id, data.user.email, portal))) {
        throw new Error(`This account cannot enter the ${title[portal].toLowerCase()}. Choose the portal assigned to your account.`);
      }
      setPassword("");
      window.location.replace(destination[portal]);
    } catch (cause) {
      if (signedInHere) { try { await supabase.auth.signOut(); } catch { /* Keep the error visible; existing route guards remain active. */ } }
      setPassword("");
  ...
  async function switchAccount() {
    ...
    else { setOtherSession(false); setPassword(""); }
    setBusy(false);
  }

  return <main className="min-h-[70vh] bg-[#f4f6f1] px-4 py-12 text-[#183125] sm:px-6">
    <div className="mx-auto w-full max-w-lg rounded-3xl border border-[#dce6d8] bg-white p-5 shadow-sm sm:p-9">
      <Link to="/portal" className="text-sm font-bold text-[#356942]">← Choose another portal</Link>
      ...
      <p className="mt-3 text-sm leading-6 text-[#617466]">{portal === "staff" ? "Employee sign-in and registration." : portal === "reception" ? "Dedicated front-desk sign-in for Reception 2.0." : "Dedicated administrator sign-in for management tools."}</p>
      {loading ? <p role="status" className="mt-6 text-sm">Checking your session…</p> : <>
        {otherSession ? <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm">
          <p>Another portal account is signed in on this browser. Sign out before switching.</p>
          <button type="button" disabled={busy} onClick={() => void switchAccount()} className="mt-3 inline-flex items-center gap-2 font-bold underline disabled:opacity-50"><LogOut size={16} /> Sign out and switch</button>
        </div> : <>
          {portal === "staff" && !recovery && <div className="mt-6 grid grid-cols-2 gap-2">
            <button type="button" onClick={() => { setRegister(false); setError(""); setNotice(""); }} className={`rounded-xl p-3 text-sm font-bold ${!register ? "bg-[#193b2a] text-white" : "bg-[#edf4e9]"}`}>Sign in</button>
            <button type="button" onClick={() => { setRegister(true); setError(""); setNotice(""); }} className={`rounded-xl p-3 text-sm font-bold ${register ? "bg-[#193b2a] text-white" : "bg-[#edf4e9]"}`}>Apply as staff</button>
          </div>}
          <form onSubmit={submit} className="mt-6 grid gap-4">
            ...
            <button type="submit" disabled={busy} className="mt-2 inline-flex items-center justify-center gap-2 rounded-xl bg-[#193b2a] p-3.5 font-bold text-white disabled:opacity-60">{busy ? "Please wait…" : recovery ? "Email reset link" : register ? "Submit staff application" : "Sign in"}<ArrowRight size={17} /></button>
          </form>
        ...
        <p className="mt-7 text-xs text-[#657769]">This portal checks your authenticated account and assigned role. The existing protected pages and database permissions still control access.</p>
      </>}
    </div>
  </main>;
}
