...
export function Navbar() {
  ...
  return <header className="sticky top-0 z-50 border-b border-border/70 bg-background/95 backdrop-blur"><div className="section-shell grid h-14 grid-cols-[minmax(0,1fr)_auto] items-center gap-3 sm:h-16 lg:grid-cols-[auto_minmax(0,1fr)_auto]">
    ...
    <div className="flex items-center gap-2"><Button asChild size="lg" className="hidden sm:inline-flex"><Link to="/join">Join now</Link></Button><Sheet open={open} onOpenChange={setOpen}><SheetTrigger asChild><Button variant="outline" size="icon" className="size-10 lg:hidden" aria-label="Open menu"><Menu /></Button></SheetTrigger><SheetContent side="right" className="h-[100dvh] w-[82vw] max-w-sm overflow-y-auto bg-secondary p-0 text-secondary-foreground"><SheetTitle className="sr-only">Navigation</SheetTitle><div className="flex min-h-[100dvh] flex-col px-5 py-4"><div className="flex items-center justify-between"><Link to="/" onClick={() => setOpen(false)} aria-label="Super Plus Fitness home"><img src="/header-logo-colour.svg" alt="Super Plus Fitness" width={1030} height={255} className="h-9 w-auto max-w-[55vw] object-contain" /></Link><Button variant="ghost" size="icon" onClick={() => setOpen(false)} aria-label="Close menu"><X /></Button></div><nav aria-label="Mobile navigation" className="mt-5 flex flex-col">{links.map(item => <Link key={item.to} to={item.to} onClick={() => setOpen(false)} className={`border-b border-primary-foreground/15 py-2 font-display text-[21px] font-bold uppercase leading-tight ${pathname === item.to ? "text-primary" : "hover:text-primary"}`}>{item.label}</Link>)}</nav><div className="mt-5 grid gap-3 text-sm font-bold uppercase"><Link to="/login" onClick={() => setOpen(false)}>Member login</Link><Link to="/my-qr" onClick={() => setOpen(false)}>My QR Code</Link><Button asChild size="lg"><Link to="/join" onClick={() => setOpen(false)}>Join now</Link></Button></div></div></SheetContent></Sheet></div>
  </div></header>;
}

export function Footer() {
  return <footer className="bg-secondary py-14 text-secondary-foreground"><div className="section-shell"><div className="grid gap-10 border-b border-primary-foreground/15 pb-12 md:grid-cols-2 lg:grid-cols-4"><div><Logo /><p className="mt-5 max-w-xs text-sm leading-6 text-secondary-foreground/60">Modern fitness, personal training, spa and recovery in Shomolu, Lagos.</p></div><div><h3 className="text-xs font-extrabold uppercase text-primary">Explore</h3><nav className="mt-5 grid gap-2">{navItems.slice(0, 8).map(item => <Link key={item.to} to={item.to} className="text-sm text-secondary-foreground/70 hover:text-primary">{item.label}</Link>)}</nav></div><div><h3 className="text-xs font-extrabold uppercase text-primary">Member &amp; Portal</h3><nav className="mt-5 grid gap-2">{[{ label: "Member login", to: "/login" }, { label: "My QR Code", to: "/my-qr" }, { label: "Staff, reception & admin portal", to: "/portal" }].map(item => <Link key={item.to} to={item.to} className="text-sm text-secondary-foreground/70 hover:text-primary">{item.label}</Link>)}</nav></div><div><h3 className="text-xs font-extrabold uppercase text-primary">Contact</h3><div className="mt-5 grid gap-3 text-sm text-secondary-foreground/70"><a href={`tel:${contact.phoneHref}`} className="hover:text-primary">{contact.phone}</a><a href={`mailto:${contact.email}`} className="break-all hover:text-primary">{contact.email}</a><p>{contact.address}</p><div className="flex gap-4"><a href="https://www.instagram.com/superplusfitnessandspa/" aria-label="Instagram" target="_blank" rel="noreferrer"><Instagram /></a><a href="https://www.tiktok.com/@superplusfitness" aria-label="TikTok" target="_blank" rel="noreferrer">TikTok</a></div></div></div></div><p className="pt-6 text-xs text-secondary-foreground/45">© {new Date().getFullYear()} Super Plus Fitness &amp; Spa. All rights reserved.</p></div></footer>;
}
