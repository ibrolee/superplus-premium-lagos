...
import { adminGroups } from "./AdminWorkspaceShell";
...
/** Pages with their own new admin shell are intentionally excluded to avoid duplicate navigation.
 * Public pages, the sign-in/reset pages, and member-facing pages are also excluded.
 */
export function isAdminDestination(pathname: string): boolean {
  return pathname.startsWith("/management-") ||
    ["/staff-admin", "/staff-blog", "/staff-gallery", "/staff-attendance", "/staff-missed-scans", "/reception-workspace", "/reception-checkin"].includes(pathname) ||
    pathname.startsWith("/reception-member/");
}

/** Shared group catalog and search destinations with the new dashboard; this only adds navigation.
 * It never supplies role permissions to the underlying page or changes a financial operation.
 */
export function AdminPersistentNavigation() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      } catch { /* Existing protected destination still handles authentication errors. */ }
    })();
  ...
  useEffect(() => {
    ...
    return () => { document.body.classList.remove("spf-admin-nav-active"); };
  }, [eligible, role]);
  ...
  useEffect(() => {
    ...
    if (!eligible || !role || !searchOpen || term.length < 2) {
      setMembers([]); setSearchError(""); setSearching(false);
      return;
    }
    let cancelled = false;
    setMembers([]);
  ...
  const closeNavigation = () => { setMobileOpen(false); setSearchOpen(false); };

  return <>
    {mobileOpen && <button type="button" aria-label="Close admin menu" onClick={() => setMobileOpen(false)} className="fixed inset-0 z-[60] bg-black/50 lg:hidden"/>}
    <aside aria-label="Persistent admin sidebar" className={`fixed inset-y-0 left-0 z-[70] flex w-[min(85vw,300px)] flex-col overflow-y-auto bg-[#152820] p-5 text-white shadow-xl transition-transform lg:w-[270px] lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      <button type="button" aria-label="Close admin menu" onClick={() => setMobileOpen(false)} className="absolute right-4 top-5 rounded-lg p-2 hover:bg-white/10 lg:hidden"><X size={20}/></button>
      <a href="/admin-workspace" onClick={closeNavigation} className="flex items-center gap-3 rounded-xl px-2 py-2">
        ...
        {adminGroups.map((group) => {
          ...
          return <div key={group.name} className="rounded-xl border border-white/10">
            ...
            {open && <div className="space-y-0.5 px-2 pb-2">{tools.map((tool) => {
              ...
              return <a key={`${tool.href}-${tool.label}`} href={tool.href} onClick={closeNavigation}
                ...
                <ToolIcon size={15} className="shrink-0"/>{tool.label}
              </a>;
            })}</div>}
          </div>;
        })}
      </nav>
      <a href="/admin-workspace" onClick={closeNavigation} className="mt-3 flex items-center gap-2 rounded-xl border border-white/20 px-4 py-3 text-xs font-bold text-[#d5e3d8] hover:bg-white/10"><LayoutDashboard size={16}/> Back to dashboard</a>
    </aside>
    <header className="sticky top-14 z-40 border-b border-[#e1e8dd] bg-[#f4f6f1]/95 px-4 py-3 backdrop-blur sm:top-16 sm:px-7 lg:px-10">
      <div className="mx-auto flex max-w-[1400px] items-center gap-3">
        <button type="button" aria-label="Open admin menu" aria-expanded={mobileOpen} onClick={() => setMobileOpen(true)} className="rounded-xl border border-[#d8e2d5] bg-white p-2.5 lg:hidden"><Menu size={21}/></button>
        <a href="/admin-workspace" className="hidden min-w-0 flex-1 text-xs font-bold text-[#526b57] sm:block">SUPER PLUS / ADMIN</a>
      ...
      {searchOpen && <div className="relative mx-auto mt-3 max-w-[1400px]"><div className="rounded-2xl border border-[#d8e2d5] bg-white p-3 shadow-lg">
        ...
        {query.trim() && <div className="mt-3 max-h-[55vh] overflow-y-auto">
          ...
          {toolResults.map((tool) => <a href={tool.href} onClick={closeNavigation} key={`${tool.href}-${tool.label}`} className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-[#edf6e7]"><ChevronRight size={15}/>{tool.label}</a>)}
          ...
          {!searching && members.map((member) => <a key={member.id} href={`/reception-member/${member.id}`} onClick={closeNavigation} className="flex items-center justify-between gap-2 rounded-lg px-3 py-2 text-sm hover:bg-[#edf6e7]"><span className="min-w-0"><strong className="block truncate">{member.full_name || "Unnamed member"}</strong><span className="block truncate text-xs text-[#66766a]">{member.phone || member.email || "No contact"}</span></span><ChevronRight size={16}/></a>)}
          ...
          <a href="/admin-members" onClick={closeNavigation} className="mt-2 block rounded-xl bg-[#edf6e7] px-3 py-3 text-xs font-bold text-[#356942]">Open full members list →</a>
        </div>}
      </div></div>}
    </header>
  </>;
