...
export function isAdminDestination(pathname: string): boolean {
  if (["/management-operations", "/management-preview", "/management-standard-plan", "/management-custom-plan", "/management-communications"].includes(pathname)) return false;
...
export function AdminPersistentNavigation() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      } catch { /* The protected destination handles authentication errors. */ }
    })();
  ...
  useEffect(() => {
    ...
    if (!eligible || !role || !searchOpen || term.length < 2) { setMembers([]); setSearchError(""); setSearching(false); return; }
    let cancelled = false;
      ...
      void (async () => {
        try {
          const { data, error } = await supabase.from("members")
            .select("id,full_name,phone,email").or(`full_name.ilike.%${term}%,phone.ilike.%${term}%,email.ilike.%${term}%`)
  ...
  return <>
    ...
    <aside aria-label="Persistent admin sidebar" className={`fixed inset-y-0 left-0 z-[70] flex w-[min(85vw,300px)] flex-col overflow-y-auto bg-[#152820] p-5 text-white shadow-xl transition-transform lg:w-[270px] lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <a href="/reception-workspace" onClick={closeNavigation} className="mt-3 rounded-xl border border-white/20 px-4 py-3 text-xs font-bold text-[#d5e3d8] hover:bg-white/10">Switch to Reception Dashboard ↗</a>
      <a href="/admin-workspace" onClick={closeNavigation} className="mt-2 flex items-center gap-2 rounded-xl border border-white/20 px-4 py-3 text-xs font-bold text-[#d5e3d8] hover:bg-white/10"><LayoutDashboard size={16}/> Back to dashboard</a>
    </aside>
