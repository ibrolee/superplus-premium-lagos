    ...
    tools: [
      {
        ...
        icon: LayoutDashboard,
        keywords: "home revenue summary overview",
      },
      {
        label: "Reception dashboard",
        href: "/reception-workspace",
        icon: UserPlus,
        keywords: "front desk daily tools registration renewal",
      },
    ],
  },
  {
    ...
    tools: [
      {
        ...
        href: "/admin-members",
        icon: Users,
        keywords: "search directory customers clients",
      },
      {
        label: "Member profiles",
        href: "/management-profiles",
        icon: UserRound,
        keywords: "history plans renewals profile",
      },
      {
        label: "Member attendance",
        href: "/management-attendance",
        icon: Activity,
        keywords: "visits check in check out records",
      },
      {
    ...
    tools: [
      {
        ...
        icon: UserPlus,
        keywords: "new member existing member renewal coupon cash pos transfer",
      },
      {
        label: "QR scanner",
        href: "/reception-checkin",
        icon: Activity,
        keywords: "member scan checkin checkout",
      },
      {
        label: "Birthdays & reminders",
      ...
      },
      {
        ...
        icon: ClipboardList,
        keywords: "front desk tools workflows",
      },
    ],
  },
  {
    ...
    tools: [
      ...
      },
      {
        ...
        icon: Users,
        adminOnly: true,
        keywords: "employee authorisation approve reject",
      },
      {
        ...
        icon: CalendarDays,
        adminOnly: true,
        keywords: "staff missed scan correction",
      },
      {
    ...
    tools: [
      {
        ...
        icon: Wallet,
        keywords: "sales payments income revenue finance",
      },
      {
        label: "Attendance export",
        href: "/management-attendance-export",
        icon: FileDown,
        keywords: "download member staff csv",
      },
      {
        label: "Monthly staff attendance",
        href: "/management-staff-monthly",
        icon: CalendarDays,
        keywords: "hours shifts monthly report",
      },
      {
        label: "QR attendance review",
        href: "/management-staff-review",
        icon: ShieldCheck,
        keywords: "employee QR exceptions anomaly",
      },
    ],
  },
  {
    ...
    tools: [
      {
        ...
        icon: Megaphone,
        keywords: "banner popup flyer notice",
      },
      { label: "Gallery", href: "/staff-gallery", icon: Images, keywords: "photos videos media" },
      { label: "Blog", href: "/staff-blog", icon: BookOpen, keywords: "articles posts website" },
    ],
  },
  {
    name: "Legacy tools",
    ...
    tools: [
      {
        ...
        icon: Wallet,
        keywords: "ledger historical baseline old",
      },
      {
      ...
      {
        label: "Full legacy console",
        href: "/staff-admin",
        icon: ShieldCheck,
        adminOnly: true,
        keywords: "original admin all tools",
      },
    ],
  },
];
export function canSeeAdminTool(tool: Tool, role: string | null) {
  return (
    !!role &&
    (!tool.adminOnly || role === "admin") &&
    (!tool.ownerOrAdmin || role === "admin" || role === "owner")
  );
}
type MemberHit = {
  id: string;
...
export function AdminWorkspaceShell({
  ...
  const [searchOpen, setSearchOpen] = useState(false),
    ...
    [members, setMembers] = useState<MemberHit[]>([]),
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!staff?.active || !["admin", "owner", "manager"].includes(next))
          throw Error("An active management account is required.");
        if (!cancelled) setRole(next);
      } catch (cause) {
        if (!cancelled)
          setAccessError(cause instanceof Error ? cause.message : "Unable to check access.");
      } finally {
  ...
  useEffect(() => {
    ...
    if (!role || !searchOpen || term.length < 2) {
      setMembers([]);
      setSearching(false);
      setSearchError("");
      return;
      ...
      >
        {adminGroups.map((group) => {
          const Icon = group.icon,
            tools = group.tools.filter((tool) => canSeeAdminTool(tool, role));
          const open =
            expanded[group.name] ?? tools.some((tool) => tool.href.split("#")[0] === active);
          return (
                  ...
                  {tools.map((tool) => {
                    ...
                    return (
                      <a
                        ...
                        className={`flex items-center gap-2 rounded-lg px-3 py-2.5 text-xs font-semibold ${active === tool.href ? "bg-[#b8ee73] text-[#193327]" : "text-[#d5e3d8] hover:bg-white/10"}`}
                      >
  ...
  if (checking)
    return (
      <main role="status" className="min-h-screen bg-[#f4f6f1] p-8 text-sm">
        Checking management access…
      </main>
    );
  if (!role)
    return (
      <main className="min-h-screen bg-[#f4f6f1] p-8">
        <div
          role="alert"
          className="mx-auto max-w-lg rounded-2xl border border-red-200 bg-white p-7 text-red-800"
        >
          {accessError || "Access denied."}{" "}
          <a href="/portal/admin" className="font-bold underline">
            Admin sign-in
          </a>
        </div>
      </main>
  ...
  return (
    <div className="min-h-screen bg-[#f4f6f1] text-[#16221c] lg:flex">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-[min(85vw,300px)] flex-col overflow-y-auto bg-[#152820] p-5 text-white transition-transform lg:sticky lg:top-0 lg:z-10 lg:h-screen lg:w-[270px] lg:shrink-0 lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <button
          type="button"
          aria-label="Close menu"
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-30 border-b border-[#e1e8dd] bg-[#f4f6f1]/95 px-4 py-3 backdrop-blur sm:px-7 lg:px-10">
          <div className="mx-auto flex max-w-[1400px] items-center gap-3">
            ...
            <span className="hidden min-w-0 flex-1 text-xs font-bold text-[#526b57] sm:block">
              SUPER PLUS / ADMIN
            </span>
            <button
              type="button"
            ...
            <div className="relative mx-auto mt-3 max-w-[1400px]">
              <div className="rounded-2xl border border-[#d8e2d5] bg-white p-3 shadow-lg">
                <label className="flex items-center gap-2 rounded-xl bg-[#f4f6f1] px-3">
                  <Search size={18} />
                  <span className="sr-only">Search admin tools and members</span>
                  <input
                    autoFocus
                    value={query}
                    onChange={(e) => setQuery(e.target.value)}
                    placeholder="Revenue, staff request, member name or phone"
                ...
                {!!query.trim() && (
                  <div className="mt-3 max-h-[55vh] overflow-y-auto">
                    ...
                    {!searching &&
                      ...
                      !members.length &&
                      !searchError && <p className="px-3 text-xs">No matching members.</p>}
                    <a
                      href="/admin-members"
