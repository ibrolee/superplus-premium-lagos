...
import { downloadMemberCardSvg } from '@/lib/member-card-svg';
...
function MembershipCardPage() {
  ...
  function exportSvg() {
    if (!member?.member_card_number || !logo || !frontRef.current || !backRef.current || busy) return;
    setError('');
    try {
      const name = member.full_name.replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '').toLowerCase();
      const prefix = `${formattedNumber(member.member_card_number)}-${name}-membership-card`;
      downloadMemberCardSvg(frontRef.current, `${prefix}-front.svg`);
      downloadMemberCardSvg(backRef.current, `${prefix}-back.svg`);
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not prepare the editable SVG files.'); }
  }
  ...
  return <main className="min-h-screen bg-[#111113] px-4 py-8 text-white sm:px-7 sm:py-12"><div className="mx-auto max-w-6xl">
    ...
    {member && <>
      {!member.member_card_number && <div className="mt-6 rounded-2xl border border-amber-400/40 bg-[#392c1e] p-5 text-sm leading-6 text-[#ffe6aa]"><strong>ID unavailable:</strong> A permanent member ID has not been assigned. PDF and SVG export and replacement are disabled until the member database is updated.</div>}
      ...
      <div className="mt-7 rounded-2xl border border-[#44444a] bg-[#232328] p-5 sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-black">Print-ready files</h2><p className="mt-1 text-sm text-[#c1c0c1]">Two-page PDF: front on page 1 and back on page 2, CR80 finished size 85.6 × 54 mm. Editable SVG downloads provide separate front and back vector files for CorelDRAW. Print at 100% scale; ask the card shop whether they require a separate bleed specification.</p><p className="mt-2 text-xs text-[#ffb6a1]">iPhone: After preparing the PDF, tap Save / Share / Print and select Save to Files or Print. For SVG, save both downloaded sides to Files.</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={busy || !logo || !member.member_card_number} onClick={exportSvg} className="inline-flex items-center gap-2 rounded-xl border border-[#ff7050] px-5 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-40"><Download size={17}/> Download Editable SVG (Front + Back)</button><button type="button" disabled={busy || !logo || !member.member_card_number} onClick={() => void exportPdf('save')} className="inline-flex items-center gap-2 rounded-xl bg-[#eb3037] px-5 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-40"><Download size={17}/>{busy ? 'Preparing…' : 'Download PDF'}</button><button type="button" disabled={busy || !logo || !member.member_card_number} onClick={() => void exportPdf('open')} className="inline-flex items-center gap-2 rounded-xl border border-[#8e8e95] px-5 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-40"><Printer size={17}/> Open to print</button></div></div></div>
