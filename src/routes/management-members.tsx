...
import {
  Activity, ArrowLeft, ArrowRight, CalendarClock, CheckCircle2, ChevronLeft,
  ChevronRight, Clock3, Loader2, RefreshCw, Search, ShieldCheck, UserPlus,
  Users, Wallet,
} from "lucide-react";
...
type MemberRow = Member & {
  plan: string;
  expiry: string | null;
  status: "active" | "expiring" | "upcoming" | "paused" | "expired" | "inactive";
};
...
function memberStatus(member: Member, plans: Membership[], today: string, sevenDays: string): MemberRow {
  const dated = [...plans].sort((a, b) => (b.end_date || "").localeCompare(a.end_date || ""));
  const valid = plans.filter((m) => {
    const status = (m.status || "").toLowerCase();
    return status !== "paused" && status !== "cancelled" && !!m.start_date && !!m.end_date && m.start_date.slice(0, 10) <= today && m.end_date.slice(0, 10) >= today;
  });
  const current = [...valid].sort((a, b) => (b.end_date || "").localeCompare(a.end_date || ""))[0];
  const upcoming = plans.find((m) => m.start_date && m.start_date.slice(0, 10) > today && !["paused", "cancelled"].includes((m.status || "").toLowerCase()));
  const chosen = current || upcoming || dated[0];
  let status: MemberRow["status"] = "inactive";
  if (current) status = current.end_date!.slice(0, 10) <= sevenDays ? "expiring" : "active";
  else if (upcoming) status = "upcoming";
  else if (dated.some((m) => (m.status || "").toLowerCase() === "paused")) status = "paused";
  else if (dated.some((m) => m.end_date && m.end_date.slice(0, 10) < today)) status = "expired";
  return { ...member, plan: chosen?.plan_name || "No membership", expiry: chosen?.end_date || null, status };
}
const label: Record<MemberRow["status"], string> = {
  active: "Active", expiring: "Expiring soon", upcoming: "Upcoming", paused: "Paused", expired: "Expired", inactive: "No active plan",
};
...
function ManagementMembers() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const verifiedRole = String(staff?.role || "").toLowerCase();
        if (!staff?.active || !["reception", "admin", "owner", "manager"].includes(verifiedRole)) {
          throw new Error("Only active reception and management accounts can access the member directory.");
        }
        if (cancelled) return;
        setRole(verifiedRole);
        const [memberRows, planRows] = await Promise.all([
          fetchAll<Member>("members", "id,full_name,email,phone,created_at"),
          fetchAll<Membership>("memberships", "id,member_id,plan_name,start_date,end_date,status"),
        ]);
        if (!cancelled) { setMembers(memberRows); setMemberships(planRows); }
      } catch (cause) {
        if (!cancelled) { setRole(null); setError(cause instanceof Error ? cause.message : "Unable to load members."); }
  ...
  const rows = useMemo(() => {
    ...
    memberships.forEach((membership) => grouped.set(membership.member_id, [...(grouped.get(membership.member_id) || []), membership]));
    const cutoff = addDays(today, 7);
    return members.map((member) => memberStatus(member, grouped.get(member.id) || [], today, cutoff))
  ...
  const tabs: { value: Filter; text: string; count: number }[] = [
    ...
    { value: "active", text: "Active", count: rows.filter((m) => ["active", "expiring"].includes(m.status)).length },
    { value: "expiring", text: "Expiring", count: rows.filter((m) => m.status === "expiring").length },
    { value: "expired", text: "Expired", count: rows.filter((m) => m.status === "expired").length },
    { value: "inactive", text: "Other", count: rows.filter((m) => ["inactive", "paused", "upcoming"].includes(m.status)).length },
  ];
  return (
      ...
      <div className="mx-auto flex min-h-screen max-w-[1680px] flex-col lg:flex-row">
        <aside className="border-b border-[#243b32] bg-[#152820] px-5 py-5 text-white lg:sticky lg:top-0 lg:flex lg:h-screen lg:w-64 lg:flex-col lg:border-b-0 lg:px-6 lg:py-8">
          ...
          <nav aria-label="Management navigation" className="mt-4 flex gap-2 overflow-x-auto pb-1 lg:flex-col lg:overflow-visible">
            ...
            <a href="/reception-dashboard" className="flex shrink-0 items-center gap-3 rounded-xl px-4 py-3 text-sm text-[#d5e3d8] hover:bg-white/10"><UserPlus size={17}/> Registration & renewals</a>
        ...
        <main className="min-w-0 flex-1 px-4 py-7 sm:px-7 lg:px-10 lg:py-10">
          ...
          {!loading && role && !error && <>
            <section aria-label="Membership overview" className="mt-7 grid gap-4 sm:grid-cols-3">{[
              ...
              { title: "Active members", value: tabs[1].count, icon: CheckCircle2 },
              { title: "Expiring in seven days", value: tabs[2].count, icon: CalendarClock },
            ...
            <section className="mt-6 rounded-[24px] border border-[#e1e8dd] bg-white p-4 sm:p-7"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black">All member records</h2><p className="mt-1 text-xs text-[#748276]">Read-only directory · registration and edits use the existing member tools.</p></div><a href="/reception-dashboard" className="inline-flex items-center gap-2 rounded-xl bg-[#1a3226] px-4 py-3 text-sm font-bold text-white"><UserPlus size={17}/> Registration & renewals</a></div>
              ...
              <p className="mt-3 text-xs text-[#748276]">{filtered.length} matching members · page {currentPage} of {totalPages}</p>
              <div className="mt-4 divide-y divide-[#e7ede4]">{visible.map((member) => <a href={`/reception-member/${member.id}`} key={member.id} className="group flex items-center gap-3 py-4 hover:text-[#356942]"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#edf5e7] text-sm font-black text-[#38673e]">{(member.full_name || "?").trim().slice(0, 1).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{member.full_name || "Unnamed member"}</p><p className="mt-1 truncate text-xs text-[#748276]">{member.phone || member.email || "No contact details"} · {member.plan}</p><p className="mt-1 text-xs text-[#748276]">Plan ends: {formatDate(member.expiry)}</p></div><span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-bold sm:px-3 sm:text-xs ${member.status === "active" ? "bg-emerald-50 text-emerald-800" : member.status === "expiring" ? "bg-amber-50 text-amber-800" : "bg-[#f1f3ef] text-[#647468]"}`}>{label[member.status]}</span><ChevronRight size={18} className="hidden shrink-0 text-[#829686] sm:block"/></a>)}{!visible.length && <p className="py-12 text-center text-sm text-[#748276]">No members match this search or filter.</p>}</div>
            ...
            <div className="mt-6 flex flex-wrap gap-4 text-sm"><a className="inline-flex items-center gap-2 font-bold text-[#356942]" href="/management-preview"><ArrowLeft size={17}/> Back to overview</a><a className="inline-flex items-center gap-2 font-bold text-[#356942]" href="/reception-dashboard">Open full member tools <ArrowRight size={17}/></a></div>
          </>}
        </main>
      </div>
    </div>
