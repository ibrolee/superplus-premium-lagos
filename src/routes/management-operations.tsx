...
function ManagementOperations() {
  ...
  const dailyActions: Action[] = [
    ...
    { label: "Public Join page", description: "New online customers can create a profile and pay through Paystack. REGOFF removes only the registration fee.", href: "/join", icon: CreditCard, eyebrow: "Online registration" },
  ...
  const managementActions: Action[] = [
    { label: "Resolve older payment requests", description: "Use only for pending records from the previous workflow. New reception payments are completed directly.", href: "/management-payment-desk#approvals", icon: ShieldCheck, eyebrow: "Legacy records" },
    { label: "Resolve older walk-in requests", description: "Use only for requests already submitted before direct registration. Do not create new daily intake here.", href: "/management-new-member-intake#review", icon: ShieldCheck, eyebrow: "Legacy records" },
  ...
  return <div className="min-h-screen bg-[#f4f6f1] text-[#16221c]"><div className="mx-auto max-w-[1440px] px-4 py-9 sm:px-8 lg:px-12 lg:py-12">
    ...
    {!loading && access && <><div className="mt-8 flex flex-wrap items-center gap-3 rounded-2xl border border-[#dce9d7] bg-white px-5 py-4 text-sm"><span className="inline-flex items-center gap-2 rounded-full bg-[#edf6e7] px-3 py-1.5 font-bold capitalize text-[#32633c]"><CheckCircle2 size={16}/> {access.role} access</span><span className="text-[#657568]">Reception registers and renews directly after confirmed payment. No admin approval queue is used for new transactions.</span></div>
      ...
      <div className="mt-11 rounded-2xl border border-[#dbe5d8] bg-[#eef5e9] p-5 text-sm leading-6 text-[#546c57]"><div className="flex items-start gap-3"><ClipboardList size={20} className="mt-0.5 shrink-0 text-[#32633c]"/><p><strong>Reception 2.0:</strong> Search first, then register or renew through the direct reception form. Successful paid transactions save a membership and a normal revenue payment immediately; historical imports stay separate.</p></div></div>
    </>}
  </div></div>;
}
