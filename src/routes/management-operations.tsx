...
function ManagementOperations() {
  ...
  const dailyActions: Action[] = [
    { label: "Register a new member", description: "New workspace form: create a member and their first membership after confirming a real POS or bank transfer.", href: "/management-standard-plan", icon: UserPlus, eyebrow: "New registration" },
    { label: "Add a standard plan", description: "Find an existing member and create a separate membership. This does not extend an existing plan.", href: "/management-standard-plan", icon: CreditCard, eyebrow: "Existing members" },
  ...
  const managementActions: Action[] = [
    { label: "Staff directory & QR attendance", description: "New read-only team overview with recorded clock-ins, daily date selector and staff status filters.", href: "/management-staff", icon: Users, eyebrow: "Management only" },
    ...
    { label: "Revenue report", description: "New read-only management revenue view with period and payment-source filters, successful transactions and historical baseline safeguards.", href: "/management-revenue", icon: Wallet, eyebrow: "Management only" },
  ...
  return <div className="min-h-screen bg-[#f4f6f1] text-[#16221c]"><div className="mx-auto max-w-[1440px] px-4 py-9 sm:px-8 lg:px-12 lg:py-12">
    ...
    {!loading && access && <><div className="mt-8 flex flex-wrap items-center gap-3 rounded-2xl border border-[#dce9d7] bg-white px-5 py-4 text-sm"><span className="inline-flex items-center gap-2 rounded-full bg-[#edf6e7] px-3 py-1.5 font-bold capitalize text-[#32633c]"><CheckCircle2 size={16}/> {access.role} access</span><span className="text-[#657568]">New staff, member and revenue views are read-only. Membership forms record payment only after staff confirms receipt.</span></div>
      ...
      <div className="mt-11 rounded-2xl border border-[#dbe5d8] bg-[#eef5e9] p-5 text-sm leading-6 text-[#546c57]"><div className="flex items-start gap-3"><ClipboardList size={20} className="mt-0.5 shrink-0 text-[#32633c]"/><p><strong>About the transition:</strong> New staff, member, attendance and revenue views remain read-only. Membership forms reuse existing payment/membership functions; original reception tools, scanning, payroll and financial reports remain available. Never create a dummy paid membership on the live database.</p></div></div>
    </>}
  </div></div>;
}
