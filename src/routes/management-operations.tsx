...
function ManagementOperations() {
  ...
  const dailyActions: Action[] = [
    { label: "Register a new member online", description: "New members register and pay on the public Join page. Payments are verified automatically with Paystack.", href: "/join", icon: UserPlus, eyebrow: "New registration" },
    { label: "Submit a membership payment", description: "Find an existing member, choose a standard plan and submit cash, bank transfer or POS for independent management verification. No premature activation.", href: "/management-payment-desk", icon: CreditCard, eyebrow: "Secure existing-member payment" },
  ...
  const managementActions: Action[] = [
    { label: "Review payment and returning-member requests", description: "Independently approve or reject requests using bank/POS verification or cash reconciliation. Approvers cannot approve their own submissions.", href: "/management-payment-desk", icon: ShieldCheck, eyebrow: "Financial approvals" },
  ...
  return <div className="min-h-screen bg-[#f4f6f1] text-[#16221c]"><div className="mx-auto max-w-[1440px] px-4 py-9 sm:px-8 lg:px-12 lg:py-12">
    ...
    {!loading && access && <><div className="mt-8 flex flex-wrap items-center gap-3 rounded-2xl border border-[#dce9d7] bg-white px-5 py-4 text-sm"><span className="inline-flex items-center gap-2 rounded-full bg-[#edf6e7] px-3 py-1.5 font-bold capitalize text-[#32633c]"><CheckCircle2 size={16}/> {access.role} access</span><span className="text-[#657568]">Reception submits payment requests; only independently verified management approvals record revenue and activate paid memberships.</span></div>
      ...
      <div className="mt-11 rounded-2xl border border-[#dbe5d8] bg-[#eef5e9] p-5 text-sm leading-6 text-[#546c57]"><div className="flex items-start gap-3"><ClipboardList size={20} className="mt-0.5 shrink-0 text-[#32633c]"/><p><strong>Reception 2.0:</strong> Submit membership payments and returning-member claims through the protected desk. Pending requests do not grant access and cannot enter revenue reports. New members register through Paystack's public Join checkout.</p></div></div>
    </>}
  </div></div>;
}