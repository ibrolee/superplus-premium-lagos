...
function ManagementOperations() {
  ...
  const dailyActions: Action[] = [
    ...
    { label: "Open member profiles", description: "NEW: search a member, inspect individual membership records and recent attendance in the redesigned staff-only view.", href: "/management-profiles", icon: UserRound, eyebrow: "Member details" },
  ...
  return <div className="min-h-screen bg-[#f4f6f1] text-[#16221c]"><div className="mx-auto max-w-[1440px] px-4 py-9 sm:px-8 lg:px-12 lg:py-12">
    ...
    {!loading && access && <><div className="mt-8 flex flex-wrap items-center gap-3 rounded-2xl border border-[#dce9d7] bg-white px-5 py-4 text-sm"><span className="inline-flex items-center gap-2 rounded-full bg-[#edf6e7] px-3 py-1.5 font-bold capitalize text-[#32633c]"><CheckCircle2 size={16}/> {access.role} access</span><span className="text-[#657568]">New profile views are read-only. Membership forms record payment only after staff confirms receipt.</span></div>
      ...
      <div className="mt-11 rounded-2xl border border-[#dbe5d8] bg-[#eef5e9] p-5 text-sm leading-6 text-[#546c57]"><div className="flex items-start gap-3"><ClipboardList size={20} className="mt-0.5 shrink-0 text-[#32633c]"/><p><strong>About the transition:</strong> Overview, directory, redesigned member profiles and attendance history remain read-only. Membership forms reuse existing payment/membership functions; original reception tools, scanning, payroll and revenue reports remain available. Never create a dummy paid membership on the live database.</p></div></div>
    </>}
  </div></div>;
}
