...
function ManagementCustomPlan() {
  ...
  async function createPlan(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!plan) throw new Error("Custom Plan is not configured in membership_plans. Contact management to configure the plan.");
      const metadata = {
      ...
      if (!data?.success) throw new Error("The membership and payment were not confirmed. Please check the existing member record before trying again.");
      setReceipt(chosen);
  ...
  return <div className="reception-responsive min-h-screen bg-[#f4f6f1] px-4 py-8 text-[#16221c] sm:px-7 lg:py-12">
    <div className="mx-auto max-w-6xl">
      ...
      {!accessLoading && authorized && <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1.5fr)_minmax(260px,1fr)]">
        ...
        <aside className="h-fit min-w-0 space-y-5 lg:sticky lg:top-6"><section className="rounded-[24px] bg-[#1a3226] p-6 text-white"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#b8ee73]">Live calculation</p><h2 className="mt-3 text-2xl font-black">Custom Plan summary</h2><dl className="mt-6 space-y-4 text-sm"><div className="flex justify-between gap-3"><dt className="text-[#c2d0c4]">Member</dt><dd className="max-w-[60%] truncate text-right font-bold">{member?.full_name || "Not selected"}</dd></div><div className="flex justify-between gap-3"><dt className="text-[#c2d0c4]">Duration</dt><dd className="font-bold">{Number.isInteger(duration) && duration > 0 ? `${duration} days` : "—"}</dd></div><div className="flex justify-between gap-3"><dt className="text-[#c2d0c4]">Starts</dt><dd className="font-bold">{niceDate(start)}</dd></div><div className="flex justify-between gap-3"><dt className="text-[#c2d0c4]">Expires</dt><dd className="font-bold">{niceDate(end)}</dd></div><div className="flex justify-between gap-3"><dt className="text-[#c2d0c4]">Plan</dt><dd className="font-bold">{Number.isFinite(amount) && amount > 0 ? naira(amount) : "—"}</dd></div><div className="flex justify-between gap-3"><dt className="text-[#c2d0c4]">Registration</dt><dd className="font-bold">{naira(fee)}</dd></div></dl><div className="mt-6 flex items-center justify-between border-t border-white/20 pt-5"><span className="text-sm text-[#c2d0c4]">Total</span><strong className="reception-money break-words text-3xl font-black text-[#b8ee73]">{naira(total)}</strong></div><p className="mt-5 text-xs leading-5 text-[#c2d0c4]">Expiry is inclusive: a one-day plan starts and ends on the same day. Existing memberships are not replaced.</p></section><a href="/management-standard-plan" className="flex items-center justify-between gap-2 rounded-2xl border border-[#dbe5d8] bg-white p-5 text-sm font-bold">Standard registration & other plans <UserPlus size={18}/></a></aside>
      </div>}
    </div>
  </div>;
}
