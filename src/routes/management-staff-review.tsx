...
export const Route = createFileRoute("/management-staff-review")({
  component: StaffAttendanceReview,
});
type Staff = {
  id: string;
  ...
  for (let offset = 0; ; offset += PAGE_SIZE) {
    ...
    if (date)
      query = query
        ...
        .lt("checked_in_at", `${nextDay(date)}T00:00:00+01:00`);
    const { data, error } = await query
      .order(date ? "checked_in_at" : "id", { ascending: true })
...
function StaffAttendanceReview() {
    ...
    async function load() {
      ...
      try {
        ...
        if (
          ...
          throw Error("Only active management accounts can review staff QR exceptions.");
        ...
        if (!cancelled)
          setError(cause instanceof Error ? cause.message : "Unable to load staff QR records.");
      } finally {
  ...
  const reviews = useMemo((): Review[] => {
    ...
    scans.forEach((scan) => {
      const group = groups.get(scan.staff_profile_id) || [];
      group.push(scan);
      groups.set(scan.staff_profile_id, group);
    });
    return staff
      .flatMap((person) => {
  ...
  const visible = reviews.filter((item) => {
    ...
    return (
      (issue === "all" ||
        (issue === "late" && item.late) ||
        (issue === "open" && item.open > 0) ||
        (issue === "overlapping" && item.overlapping > 0) ||
        (issue === "invalid" && item.invalid > 0)) &&
      (!term ||
        [item.staff.full_name, item.staff.staff_id, item.staff.position].some((value) =>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black tracking-tight sm:text-5xl">
              QR attendance review
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-7 text-[#647468]">
              Find recorded late arrivals, unfinished shifts, overlapping clock-ins and invalid
              sessions for a selected day.
            </p>
          </div>
          <button
            type="button"
        ...
        {!loading && authorized && (
          <>
            <div className="mt-8 flex items-start gap-3 rounded-2xl border border-[#d9e6d2] bg-[#eef6e9] p-5 text-sm leading-6 text-[#476149]">
              ...
              <p>
                <strong>Review only — no penalties or edits.</strong> The 7:30 AM Monday–Saturday
                rule preserves both named exceptions. An open session on the current date may still
                be in progress. Records are grouped by their recorded clock-in date in Lagos; a
                missing scan does not prove absence. Overlapping completed sessions are counted only
                once in worked hours, and incomplete or invalid sessions are excluded. Verify QR
                records and missed-scan reports separately before payroll decisions.
              </p>
            </div>
            <section
            ...
            >
              <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                  <h2 className="text-xl font-black">Recorded exceptions</h2>
                  <p className="mt-2 text-xs text-[#718172]">
                    {scans.length} sessions loaded · {staff.length} staff profiles
                  </p>
                </div>
                <label className="text-xs font-bold text-[#607465]">
                  <span className="mb-2 block">Review date · Lagos</span>
                  <input
                    type="date"
                    value={date}
                    max={lagosToday()}
                    onChange={(event) => {
                      ...
                      if (/^\d{4}-\d{2}-\d{2}$/.test(value) && value <= lagosToday())
                        setDate(value);
                    }}
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                {[
                  ...
                  {
                    label: "Late arrivals",
                    value: reviews.filter((item) => item.late).length,
                    color: "text-red-700",
                  },
                  {
                    label: "Open sessions",
                    value: reviews.reduce((total, item) => total + item.open, 0),
                    color: "text-amber-700",
                  },
                  {
                    label: "Overlapping sessions",
                    value: reviews.reduce((total, item) => total + item.overlapping, 0),
                    color: "text-red-700",
                  },
                  {
                    label: "Invalid sessions",
                    value: reviews.reduce((total, item) => total + item.invalid, 0),
                    color: "text-amber-700",
                  },
                ].map((item) => (
                  <div
                    key={item.label}
                    className="rounded-2xl border border-[#e2e9dd] bg-[#f8faf6] p-4"
                  >
                    <p className="text-xs font-bold text-[#607465]">{item.label}</p>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <label className="relative min-w-0 flex-1">
                  ...
                  />
                  <span className="sr-only">Search staff for attendance review</span>
                  <input
                    value={query}
                ...
                <label>
                  <span className="sr-only">Filter exception type</span>
                  <select
                    value={issue}
                  ...
                  >
                    ...
                    <option value="invalid">Invalid sessions</option>
                  </select>
                </label>
              </div>
              <div className="mt-5 divide-y divide-[#e7ede4]">
                {visible.map((item) => (
                  <div key={item.staff.id} className="flex flex-wrap items-start gap-4 py-5">
                    <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-red-50 text-red-700">
                      <AlertTriangle size={20} />
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="font-black">{item.staff.full_name || "Unnamed staff"}</p>
                      <p className="mt-1 text-xs text-[#718172]">
                        {item.staff.staff_id || "No staff ID"} ·{" "}
                        {item.staff.position || item.staff.status || "Staff"} · {item.scans.length}{" "}
                        recorded QR sessions
                      </p>
                      <p className="mt-2 text-xs font-medium text-[#607465]">
                        First clock-in: {lagosClock(item.firstIn)} · Completed work:{" "}
                        {workDuration(item.minutes)} ({item.completed} completed)
                      </p>
                      <div className="mt-3 flex flex-wrap gap-2">
                        ...
                        {item.open > 0 && (
                          <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-800">
                            {item.open} open{" "}
                            {date === lagosToday() ? "· may still be working" : "· needs review"}
                          </span>
                        )}
                        {item.overlapping > 0 && (
                          <span className="rounded-full border border-red-200 bg-red-50 px-3 py-1 text-xs font-black text-red-800">
                            {item.overlapping} overlap · no double-counting
                          </span>
                        )}
                        {item.invalid > 0 && (
                          <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-800">
                            {item.invalid} invalid · excluded
                          </span>
                        )}
                      </div>
                    </div>
                    <a
                      ...
                      className="inline-flex shrink-0 items-center gap-1 rounded-xl border border-[#d8e2d5] px-3 py-2 text-xs font-bold text-[#356942]"
                    >
                      Monthly details <ArrowRight size={14} />
                    </a>
                  </div>
                ))}
                {visible.length === 0 && (
                  <p className="py-10 text-center text-sm text-[#748276]">
                    {reviews.length
                      ? "No recorded exceptions match your filters."
                      : "No late, open, overlapping or invalid QR sessions were found for this date. Missing scans still require separate review."}
                  </p>
                )}
              </div>
            </section>
