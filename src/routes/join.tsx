...
// The coupon must be enabled only after BOTH public Paystack Edge functions have been updated and tested.
const regoffEnabled = import.meta.env.VITE_REGOFF_ENABLED === 'true';
function JoinPage() {
 ...
 const cleanCoupon=coupon.trim().toUpperCase();const couponValid=cleanCoupon==='REGOFF';const couponInvalid=!!cleanCoupon&&!couponValid;
 const couponUnavailable=couponValid&&!regoffEnabled;
 const registrationFee=couponValid&&regoffEnabled?0:selectedPlan.registration;
 ...
 async function handleSubmit(event:FormEvent<HTMLFormElement>){event.preventDefault();setError('');
  ...
  if(couponInvalid){setError('Invalid code. Enter REGOFF or remove the coupon.');return;}
  if(couponUnavailable){setError('REGOFF checkout is not enabled in this preview yet. Remove the code to pay the normal price.');return;}
  setLoading(true);
  try{const {data,error:fnError}=await supabase.functions.invoke('initialize-public-payment',{body:{planId:selectedPlan.id,fullName:name,email:mail,phone:mobile,birthDay:day,birthMonth:month,...(regoffEnabled&&couponValid?{couponCode:'REGOFF'}:{})}});
 ...
 <label className="block text-sm font-medium">Coupon code (optional)<input type="text" autoComplete="off" value={coupon} onChange={e=>setCoupon(e.target.value)} disabled={loading} placeholder="REGOFF" className="mt-2 h-12 w-full rounded-xl border bg-background px-4 text-sm outline-none focus:border-primary"/>{couponValid&&regoffEnabled&&<span className="mt-2 block text-xs font-semibold text-green-700">REGOFF applied. Registration fee waived.</span>}{couponUnavailable&&<span className="mt-2 block text-xs text-amber-800">REGOFF is preview-only for now; no discount can be charged until checkout verification is ready.</span>}{couponInvalid&&<span className="mt-2 block text-xs text-destructive">Invalid code.</span>}</label>
 <div className="space-y-3 rounded-xl bg-muted/30 p-4 text-sm"><p className="flex justify-between gap-4"><span>Membership</span><strong>{formatNaira(selectedPlan.price)}</strong></p><p className="flex justify-between gap-4"><span>Registration</span><strong>{formatNaira(registrationFee)}</strong></p><p className="flex justify-between gap-4 border-t pt-3 text-lg font-bold"><span>Total</span><span>{formatNaira(totalAmount)}</span></p></div><p className="text-xs leading-5 text-muted-foreground">REGOFF removes only the registration fee; you still pay the membership price. Use your existing email if you have previously joined.</p>
 ...
 <Button type="submit" size="lg" className="h-12 w-full" disabled={loading||couponInvalid||couponUnavailable}>{loading?<><Loader2 className="mr-2 size-4 animate-spin"/>Connecting to Paystack…</>:<>Continue to payment <ArrowRight className="ml-2 size-4"/></>}</Button><p className="text-center text-xs leading-5 text-muted-foreground">You will be redirected to Paystack to complete payment securely.</p>
