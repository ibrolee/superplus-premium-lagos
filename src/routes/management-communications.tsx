...
function ManagementCommunications() {
  ...
  return <main className="min-h-screen bg-[#f4f6f1] px-4 py-8 text-[#16221c] sm:px-8"><div className="mx-auto max-w-6xl">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><a href="/reception-dashboard" className="inline-flex items-center gap-2 text-sm font-bold text-[#356942]"><ArrowLeft size={16}/> Reception Dashboard</a><p className="mt-7 text-xs font-black uppercase tracking-[.2em] text-[#62905b]">Super Plus / Reception</p><h1 className="mt-2 text-3xl font-black tracking-tight sm:text-5xl">Member communications</h1><p className="mt-3 max-w-2xl text-sm leading-7 text-[#647468]">Warm birthday wishes and friendly renewal reminders, with a shared record of messages your team confirms as sent.</p></div><button type="button" disabled={loading} onClick={() => setReload((value) => value + 1)} className="inline-flex items-center gap-2 rounded-xl border border-[#d8e2d5] bg-white px-4 py-3 text-sm font-bold disabled:opacity-50"><RefreshCw size={16} className={loading ? "animate-spin" : ""}/> Refresh</button></div>
